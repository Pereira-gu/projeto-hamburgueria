document.addEventListener('DOMContentLoaded', function() {
    // Lê as variáveis globais que o PHP nos deu no footer.php
    ...
    function atualizarQuantidade(carrinhoId, quantidade, tableRow) {
        ...
        fetch(`${baseUrl}/actions/atualizar_carrinho.php`, { method: 'POST', body: formData })
        .then(response => response.json())
        .then(data => {
            if (data.sucesso) {
                const cartCount = document.getElementById('cart-count');
                ...
                if (data.carrinho_vazio) {
                    document.getElementById('conteudo-carrinho').innerHTML = `
                        <div class="carrinho-vazio">
                            ...
                            <a href="${baseUrl}/index.php" class="btn-login" style="margin-top: 20px;">Ver cardápio</a>
                        </div>`;
